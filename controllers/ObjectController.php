...
class ObjectController extends Controller
{
    ...
    public function actionListStreets($id)
    {
        $countstreet = Streets::find()->where(['id_city'=>$id])->count();
        $street = Streets::find()->where(['id_city'=>$id])->orderBy('id DESC')->all();
        if($countstreet>0)
        {
            echo "<option value = ''>Улица не выбрана</option>";
            foreach($street as $result) echo "<option value = '".$result->id."'>".$result->name."</option>";
        }else{
            echo "<option>-</option>";
        }
    }
